...
4. Import dataset locally (optional, large files are git‑ignored):
   - Place `truestate_assignment_dataset.csv` at `backend/truestate_assignment_dataset.csv` (ignored by Git).
   - Run `node backend/src/utils/seedData.js` to populate `transactions`.
5. Start backend: `cd backend && npm start` (listens on `http://localhost:5000`).
